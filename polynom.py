...
class Polynom:
    ...
    def bisection_method(self, a, b, epsilon, pr=False):
        ...
        while math.fabs(b-a) > epsilon or math.fabs(self.function(x)) > epsilon:
            ...
            x = (a + b)/2
            iters += 1
            if pr: print(iters, 'iteration, a =', round(a, 6), 'b =', round(b, 6), 'f(a) =', round(self.function(a), 6), 'f(b) =', round(self.function(b), 6))
        if pr: print('Number of iterations:', iters)

        return x

    def chord_method(self, a, b, epsilon, pr=False):
        if pr: print('-'*70 + '\n' + ' ' * 28 + 'Hordes method' + ' ' * 29 + '\n' + '-'*70)
        f_a = self.function(a)
        f_b = self.function(b)
        x_prev = 1000000
        ...
        while math.fabs(f_x) > epsilon or math.fabs(x-x_prev) > epsilon:
            ...
            f_x = self.function(x)
            iters += 1
            if pr: print(iters, 'iteration, a =', round(a, 6), 'b =', round(b, 6), 'f(a) =', round(f_a, 6), 'f(b) = ', round(f_b, 6))
        if pr: print('Number of iterations:', iters)

        return x

    def newtons_method(self, a, b, epsilon, pr=False):
        ...
        while math.fabs(dx) > epsilon or math.fabs(self.function(x)) > epsilon:
            ...
            dx = self.function(x) / self.derivative(x)
            iters += 1
            if pr: print(iters, 'iteration, x =', x, 'f(x) =', self.function(x))
        if pr: print('Number of iterations:', iters)
...
if __name__ == "__main__":
    ...
    print('x =', round(var.bisection_method(-3, 0, 1e-5, pr=True), 5))
    print('x =', round(var.chord_method(-3, -0, 1e-5, pr=True), 5))
    print('x =', round(var.newtons_method(-3, 0, 1e-5, pr=True), 5))
